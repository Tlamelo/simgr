...
Service.prototype.getName = function (metadata) {
...
Service.prototype.getQuality = function (metadata) {
  var identity = metadata.identity
  return parseInt(identity.Quality || identity['JPEG-Quality'], 10)
}

Service.prototype.saveFile = function (stream, callback) {
  var location = path.join(tmpdir, 'irp_' + Math.random().toString(36).substring(2))
