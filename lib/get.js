...
Simgr.prototype.resizeImagemagick = function (metadata, options, callback) {
  ...
  if (variant.resize) {
    if (metadata.colorspace === 'Gray') {
      // Assumes Grayscale is a linear colorspace.
      // Changes colorspace first as
      // the original image doesn't necessarily have
      // to be grayscale.
      args.push(
        '-colorspace', 'Gray',
    ...
    args.push('-colorspace', metadata.colorspace === 'Gray'
      ? 'Gray'
      : 'sRGB'
    )
  }
