...
// NOTE:
// This method is destructive to metadata
...
Get.prototype.end = function (callback) {
  ...
  if (metadata.Format === 'GIF' && format === 'gif')
    return this.proxyFromStore(callback)
...
Get.prototype.proxyFromStore = function (callback) {
  this.simgr.getFromStore(this.metadata.name, callback)
}

// Download the entire file so we can process it however we'd like
Get.prototype.getFromStore = function (callback) {
  var metadata = this.metadata
  var simgr = this.simgr

  simgr.getFromStore(metadata.name, function (err, res) {
    if (err)
      return callback(err)
...
Get.prototype.getSignatures = function (callback) {
  ...
  function getImageHash(done) {
    ...
    // Will also error if delegate libraries not present
    // ie webp or tiff
    if (gifOutputs[format] || !Simgr.supportedInputFormat(format))
      return done()
...
Get.prototype.checkVariant = function () {
  ...
  if (!variant)
    throw simgr.error('undefined-variant')
  ...
  if (!format)
    throw simgr.error('unsupported-output-format')
  ...
  if (gifOutputs[format] && !variant[format])
    throw simgr.error('invalid-variant')
}
